        ...
        <div className="testimonios-modern-photos">
          {testimonios.map((t, idx) => (
            <img
              ...
              src={t.imagen}
              alt={t.nombre}
              className={`testimonios-modern-photo ${idx === activo ? 'activo' : idx === (activo + 1) % testimonios.length ? 'derecha' : idx === (activo - 1 + testimonios.length) % testimonios.length ? 'izquierda' : 'oculto'}`}
              style={{ zIndex: idx === activo ? 3 : 2 - Math.abs(activo - idx) }}
            />
          ))}
        </div>
